#!/usr/bin/env python3
# https://github.com/pashazz/ksetwallpaper/blob/master/LICENSE.txt
# import time
import dbus
# import argparse
# import glob
...
class KWallAPI:
    ...
    def setwallpaper(self, filepath, plugin='org.kde.image', desktop="all"):
        if desktop =="all":
            jscript = """
            ...
            var allDesktops = desktops();
            print (allDesktops);
            d = allDesktops["""+str(desktop)+"""];
            d.wallpaperPlugin = "%s";
            d.currentConfigGroup = Array("Wallpaper", "%s", "General");
            d.writeConfig("Image", "file://%s")
            """
        else:
            raise Exception("Integer input expected for setwallpaper desktop") 
    ...
    def set_lockscreen_wallpaper(self, filepath,plugin='org.kde.image'):
